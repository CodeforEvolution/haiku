...
public:
								GraphicsDriver(BMessage* message,
	...
	static	BPoint				GetScale(int32 nup, BRect physicalRect,
									float scaling);
	static	BPoint				GetOffset(int32 nup, int index,
									JobData::Orientation orientation,
									const BPoint* scale,
									BRect scaledPhysicalRect,
...
protected:
	...
	static	void				ConvertToRGB24(const void* src, void* dst,
									int width, color_space cs);
	static	void				ConvertToGray(const void* src, void* dst,
									int width, color_space cs);
...
private:
			...
			void				_RotateInto(BBitmap* target,
	...
	static	void				_ConvertRGB32ToRGB24(const void* src, void* dst,
									int width);
	static	void				_ConvertCMAP8ToRGB24(const void* src, void* dst,
									int width);
	static	uint8				_ConvertToGray(uint8 r, uint8 g, uint8 b);
	static	void				_ConvertRGB32ToGray(const void* src, void* dst,
									int width);
	static	void				_ConvertCMAP8ToGray(const void* src, void* dst,
									int width);
