 ...
 */
#ifndef _PRINTER_CAP_H
#define _PRINTER_CAP_H
...
struct EnumCap : public BaseCap {
								EnumCap(const BString& label, bool isDefault);
...
struct PaperCap : public EnumCap {
								PaperCap(const BString& label, bool isDefault,
									JobData::Paper paper,
									const BRect& paperRect,
...
struct PaperSourceCap : public EnumCap {
								PaperSourceCap(const BString& label,
									bool isDefault,
...
struct ResolutionCap : public EnumCap {
								ResolutionCap(const BString& label,
									bool isDefault, int32 id, int xResolution,
									int yResolution);
...
struct OrientationCap : public EnumCap {
								OrientationCap(const BString& label, bool isDefault,
...
struct PrintStyleCap : public EnumCap {
								PrintStyleCap(const BString& label, bool isDefault,
...
struct BindingLocationCap : public EnumCap {
								BindingLocationCap(const BString& label,
									bool isDefault,
...
struct ColorCap : public EnumCap {
								ColorCap(const BString& label, bool isDefault,
									JobData::Color color);
...
struct ProtocolClassCap : public EnumCap {
								ProtocolClassCap(const BString& label,
...
struct DriverSpecificCap : public EnumCap {
			enum Type {
				...
				kDoubleRange
								...
								DriverSpecificCap(const BString& label,
...
struct ListItemCap : public EnumCap {
								ListItemCap(const BString& label,
...
struct BooleanCap : public BaseCap {
								BooleanCap(const BString& label,
									bool defaultValue);
...
struct IntRangeCap : public BaseCap {
								IntRangeCap(const BString& label, int lower,
...
struct DoubleRangeCap : public BaseCap {
								DoubleRangeCap(const BString& label,
...
public:
	...
	struct IDPredicate
	{
		...
		bool
		operator()(const BaseCap* baseCap)
		{
	...
	virtual	const BaseCap**		GetCaps(CapID category) const = 0;
			...
			const BaseCap*		FindCap(CapID category, const char* label)
									const;
...
protected:
	...
	const	BaseCap*			FindCap(CapID category, Predicate& predicate)
									const;
