...
Resource::Resource(const char* transport, const char* address,
...
bool
Resource::Equals(const char* transport, const char* address,
	const char* connection)
{
...
Resource*
ResourceManager::_Find(const char* transport, const char* address,
	const char* connection)
{
	for (int i = 0; i < fResources.CountItems(); i++) {
		Resource* resource = fResources.ItemAt(i);
		if (resource->Equals(transport, address, connection))
			return resource;
	}
...
Resource*
ResourceManager::Allocate(const char* transport, const char* address,
	const char* connection)
{
	Resource* resource = _Find(transport, address, connection);
	if (resource == NULL) {
		resource = new Resource(transport, address, connection);
		fResources.AddItem(resource);
	} else
		resource->Acquire();
...
void
ResourceManager::Free(Resource* resource)
{
	if (resource->Release())
		fResources.RemoveItem(resource);
}
