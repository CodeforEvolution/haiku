...
PrinterSettings::PrinterSettings(const char* printer, BMessage* pageSettings,
	BMessage* jobSettings)
	:
	fPrinter(printer)
{
...
void
Settings::RemoveAppSettings(int index)
{
	delete fApps.RemoveItemAt(index);
}


void
Settings::RemovePrinterSettings(int index)
{
	delete fPrinters.RemoveItemAt(index);
}
...
Settings::Save(BFile* file)
{
	...
	for (int i = 0; i < PrinterSettingsCount(); i++) {
		PrinterSettings* printerSettings = PrinterSettingsAt(i);
		message.AddString("P", printerSettings->GetPrinter());
		message.AddMessage("S", printerSettings->GetPageSettings());
		message.AddMessage("J", printerSettings->GetJobSettings());
	}
...
Settings::Load(BFile* file)
{
	...
	if (message.Unflatten(file) == B_OK) {
		...
		for (int i = 0; message.FindString("message", i, &mimetype) == B_OK
			...
			AddAppSettings(new AppSettings(mimetype.String(),
				printer.String()));
		}
		...
		for (int i = 0; message.FindString("P", i, &printer) == B_OK &&
			...
			AddPrinterSettings(new PrinterSettings(printer.String(), &page,
				&job));
		}
