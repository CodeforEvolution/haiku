...
Transport::Scan(directory_which which)
{
	...
	result = path.Append("Print/transport");
	if (result != B_OK)
		return result;

	BDirectory dir;
	result = dir.SetTo(path.Path());
	if (result != B_OK)
		return result;
	...
	fFeatures(0)
{
	...
	if (get_image_symbol(id, B_TRANSPORT_FEATURES_SYMBOL, B_SYMBOL_TYPE_DATA,
		(void**)&transportFeaturesPointer) != B_OK)
		unload_add_on(id);
	else {
...
status_t
Transport::ListAvailablePorts(BMessage* message)
{
	...
	result = get_image_symbol(id, B_TRANSPORT_LIST_PORTS_SYMBOL,
		B_SYMBOL_TYPE_TEXT, (void**)&list_ports);
	if (result != B_OK)
		goto done;

	// run addon...
	result = (*list_ports)(message);
...
// ---------------------------------------------------------------
void
Transport::MessageReceived(BMessage* message)
{
	switch (message->what) {
		case B_GET_PROPERTY:
		case B_SET_PROPERTY:
