...
// output of a media_node.
#ifndef _MEDIA_OUTPUT_INFO_H
...
public:
	MediaOutputInfo(BBufferProducer* _node, const char* name);
	virtual ~MediaOutputInfo();
...
private:
	...
	media_format fWildcardedFormat;

	// This format provides default values for all fields.
	// These defaults are used to resolve all wildcards.
	media_format fFullySpecifiedFormat;

	// FIXME: Do we need media_seek_tag in here?
};
