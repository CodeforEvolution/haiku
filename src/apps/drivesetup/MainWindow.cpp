...
static int kDiskTaskViewHeight = 85;
...
//	fStatusView->AddChild(fStatusBar);
	...
	// add DiskTaskView
	r.top = r.bottom + 2;
	r.bottom = Bounds().bottom + kDiskTaskViewHeight;
	fDiskTaskView = new DiskTaskView(r);

	// add DiskTaskView to a BScrollView
	fDiskTaskScrollView = new BScrollView("DiskTaskScrollView", fDiskTaskView,
		B_FOLLOW_LEFT_RIGHT | B_FOLLOW_BOTTOM, 0, true, true);
	AddChild(fDiskTaskScrollView);
...
//				fListView->ResizeBy(0, -kStatusViewHeight);
		...
		case B_SAVE_REQUESTED:
		{
			...
			BEntry entry(&entryRef);
			BPath targetFolder;
			entry.GetPath(&targetFolder);
			//msg->AddString("targetfolder", targetFolder.Path());

			const char* name;
			message->FindString("name", &name);
			BPath target(targetFolder);
			target.Append(name);
			...
			//msg->AddString("target", target.Path());
			//msg->AddInt32("partitionID", row->ID());
			//msg->AddMessenger("messenger", BMessenger(this));

			fDiskTaskView->CreateSaveTask(row->DevicePath(), target.Path(),
				BMessenger(this), targetFolder.Path(), row->ID());
...
MainWindow::QuitRequested()
{
	...
	if (fDiskTaskView->AreTasksRunning()) {
		BAlert* alert = new BAlert(B_TRANSLATE("Wait a moment!"),
			B_TRANSLATE("There are still disk tasks running!\n"
				"Quitting DriveSetup will cancel these tasks!"),
			B_TRANSLATE("Cancel the tasks"), B_TRANSLATE("Wait"), NULL,
			B_WIDTH_AS_USUAL, B_EVEN_SPACING, B_WARNING_ALERT);

		int32 buttonIndex = alert->Go();
		if (buttonIndex == 0)
			fDiskTaskView->CancelAllTasks();
		else
			close = false;
