...
JobData::JobData(BMessage *message, const PrinterCap* printerCap,
	...
	SettingType settingType)
{
	...
	if (message->HasFloat(kJDGamma))
		fGamma = fMessage->FindFloat(kJDGamma);
	else
		fGamma = 0.25f;

	if (message->HasFloat(kJDInkDensity))
		fInkDensity = fMessage->FindFloat(kJDInkDensity);
	else
		fInkDensity = 0.0f;

	if (message->HasInt32(kJDPaperSource))
		fPaperSource = (PaperSource)fMessage->FindInt32(kJDPaperSource);
