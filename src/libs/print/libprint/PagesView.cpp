...
PagesView::PagesView(BRect frame, const char* name, uint32 resizeMask,
	uint32 flags)
	:
...
void
PagesView::GetPreferredSize(float* width, float* height)
{
	*width  = kWidth;
	*height = kHeight;
}
...
void
PagesView::_DrawPages(BPoint position, int number, int count)
{
...
void
PagesView::_DrawPage(BPoint position, int number)
{
	...
	DrawChar('0' + number, position +
		BPoint(kPageWidth - kPageHorizontalIndent + 1, kPageHeight - 2));
}


void
PagesView::SetCollate(bool collate)
{
	fCollate = collate;
	Invalidate();
}
...
void
PagesView::SetReverse(bool reverse)
{
	fReverse = reverse;
	Invalidate();
}
